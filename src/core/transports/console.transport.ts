...
import { LogLevel, DEFAULT_LEVEL_COLORS, DEFAULT_LEVEL_ICONS } from '../logger.config';
...
export interface ConsoleTransportOptions {
  ...
  /**
   * Whether to use relative time for recent logs
   */
...
export class ConsoleTransport implements Transport {
  ...
  constructor(options: ConsoleTransportOptions = {}) {
    ...
    this.levelColors = { ...DEFAULT_LEVEL_COLORS, ...(options.levelColors || {}) };
  ...
  public log(level: string, message: string, timestamp: string, context: Record<string, any>): void {
    ...
    const colorizedTimestamp = this.colorize ? chalk.gray(displayTimestamp) : displayTimestamp;
    
    // Format level and message
    const colorizedLevel = this.colorize ? this.levelColors[logLevel](level.toUpperCase()) : level.toUpperCase();
    const colorizedMessage = this.colorize ? chalk.whiteBright(message) : message;
    ...
    if (Object.keys(context).length) {
      for (const [key, value] of Object.entries(context)) {
        ...
        const colorizedValue = this.colorize ? chalk.cyan(formattedValue) : formattedValue;
        consoleOutput += `\n    • ${colorizedKey}: ${colorizedValue}`;
      }
    }
    
    const divider = this.colorize ? chalk.dim('\n────────────────────────────────────') : '\n────────────────────────────────────';
